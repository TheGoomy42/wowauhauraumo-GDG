...
/**
 * Runs encounter (FF1-style) battles on top of the play screen.
 * 
 * @author TheGoomy42
 */
// TODO  use TableLayout to arrange the table so you don't have to use code!
public class BattleScreen implements Screen {
    ...
    public void show() {
        ...
        // TODO create table in TableLayout editor!
        ...
        // TODO move UI atlas and skin to Atlas
        ...
        options.add(new Option<>(Choice.FIGHT, skin));
        options.add(new Option<>(Choice.MAGIC, skin));
        options.add(new Option<>(Choice.DRINK, skin));
        options.add(new Option<>(Choice.ITEM, skin));
        options.add(new Option<>(Choice.RUN, skin));

        for(int i = 0; i < options.size - 1; i++) {
            // add cursor column
            fightTable.add(options.get(i).getTable()).left();
            if(i == 0)
                fightTable.add(options.get(4).getTable()).padLeft(5).left(); // the run option is on the first row too
            fightTable.row();
        }
    ...
    /**
     * (Should be) run after all characters have selected the option. This method performs a selected action, using the
     * stats from the provided character (in the format party[character]).
     *
    ...
    /**
     * Change the displayed message. Needs to take into account the time since the last message, and wait until it has
     * expired.
     *
    ...
    private void setHPTable(PartyMember[] party) {
        for(PartyMember aParty : party) {
            HPTable.add(new Label(aParty.getName(), skin)).pad(5).padLeft(20);
            HPTable.row();
            ...
            subTable.row();
            subTable.add(new Label("" + aParty.getCurrentHP(), skin)).padLeft(60);
    ...
    private void setPartyTable(PartyMember[] party) {
        ...
        for(PartyMember pm : party) {
            try {
                friendlySpritesTable.add(new BattleActor(pm.getJob().spritePrefix.getTexture())).pad(5);
            } catch(Exception e) {
                e.printStackTrace();
            }
    ...
    private class BattleActor extends Actor {
        ...
        public BattleActor(String spritePrefix) throws Exception {
            this.sprite = Atlas.getBattleTexture(spritePrefix + '0');
            if(this.sprite == null) {
                throw new Exception("Could not find texture in Atlas: " + spritePrefix + '0');
            }
